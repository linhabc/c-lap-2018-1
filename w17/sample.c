...
{ /* enumerate states from current state, and add them into array of children states */
  ...
  {
    if (((state >> (4 * i)) & 0xF) == 0)
    {                  /* find out whether index of blank is */
  ...
  case 8:
    ...
    break;
  default:
    log_info("invalid blank index %d", blank_index);
  }
}
...
{ /* given f_score, insert board into priority queue */
  ...
  if (priorityQp->min_index > f_score || /* if inserting state with lower f_score */
      priorityQp->min_index == -1)
  {                                  /* or if min_index not yet set */
...
{ /* extract minimum board from priority queue, and update priority queue */
  ...
  while (priorityQp->queue[priorityQp->min_index] == NULL)
  { /* if index is empty */
...
{ /* remove state from priority queue */
  ...
  { /* if state at top of the linked list */
    priorityQp->queue[f_score] = current->next;
    free(current);
    priorityQp->nelements--; /* decrease number of elements */
    if (priorityQp->nelements == 0)
      priorityQp->min_index = -1; /* if no elements in priority queue, reset min index */
    return true;
  }

  while (current->next->state != state)
  { /* if state not at the top, traverse linked list */
...
{ /* search closed set for state:
  ...
  while (true)
  {
    ...
    { /* if found */
      if (closed[index].processed == true)
      {                 /* if processed */
        return INT_MAX; /* do nothing */
      }
      else
      {
        if (f_score >= closed[index].f_score)
        {                 /* f_score higher than existing */
          return INT_MAX; /* do nothing */
        }
        else
        {
...
{ /* search closed set for state and set to processed */
  ...
  while (true)
  {
    ...
    {
      if (closed[index].processed == true)
        log_info("state already processed");
...
{ /* f_score = number of misplaced tiles + number of moves made */
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    {
      if (((state & mask) == 0) && ((END_STATE & mask) == 0))
      { /* if blank matches */
        blank_misplaced = false;
      }
    }
    if (!((extract_same & mask) == 0))
    { /* if 4-bit pattern do not match */
      misplaced++;
    }
    mask = mask << 4;
  }
  ...
  if (blank_misplaced == true)
    misplaced--; /* do not include count for misplaced blank */
...
{ /* f_score =  manhattan distance + nmoves */
  ...
  for (tile = 1; tile < 9; tile++)
  {
    ...
    while (i < 9)
    {
      if ((((state >> (4 * i)) ^ tile) & mask) == 0)
        index1 = i; /* extract tile index for state */
      if ((((END_STATE >> (4 * i)) ^ tile) & mask) == 0)
        index2 = i; /* extract tile index for end state */
      i++;
    }
...
{ /* performs one step of a_star */
  ...
  if (next_boardp == NULL)
    log_info("error, failed to extract from priority queue");

  if (next_boardp->state == END_STATE)
  { /* solved */
    return next_boardp;
  }
  ...
  for (i = 0; i < next_boardp->nchildren; i++)
  {                                                                      /* for children of extracted state */
    ...
    {
      if (f_score != aux_f_score)
      { /* need to replace in priority queue */
